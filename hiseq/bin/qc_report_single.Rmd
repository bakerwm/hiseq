...
```{r samples, eval = T}
qc_dir <- "../../test/results/atac2/pe_rep1_1/qc/"
...
mito_pct <- round(df_table$mito.pct * 100, 2)
...
df4 <- data.frame(id = gsub("_peaks.narrowPeak", "", basename(peak_files)),
                 count  = sapply(gr_list, length)) %>%
  mutate(id = forcats::fct_rev(id))

p1 <- barplot2(df3, TRUE) + 
  ggtitle("Unique reads on genome") +
  ylab("Million of reads")

p2 <- barplot2(df4, TRUE) + 
