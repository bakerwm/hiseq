...
"""
Alignment: level-2 (generate report)
Generate report
"""
...
class AlignRp(object):
    ...
    >>> AlignRp(project_dir).run()
    >>> AlignRp(project_dir).report()

    $ python alignment_rp.py -i prj_dir
    """
    ...
    def report(self):
        ...
        align_report_html = os.path.join(
            self.report_dir,
            'HiSeq_report.html')
        cmd = ' '.join([
        ...
        with open(cmd_txt, 'wt') as w:
            w.write(cmd + '\n')
        # report_html
        if file_exists(align_report_html) and not self.overwrite:
...
def get_args():
    example = '\n'.join([
        'Examples:',
        '$ python alignmen t_rp.py -i project_dir',
    ])
