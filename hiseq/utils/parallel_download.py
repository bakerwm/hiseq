...
class Downloader:
    ...
    def __init__(self, url=None, number_of_threads=1, target_file=None):
...
#         self.target_filename = os.path.basename(self.url)  # name of a file to be downloaded
        if isinstance(target_file, str):
            ...
            fname = get_file_name(self.url)
            self.target_filename = os.path.join(str(pathlib.Path.cwd()), fname)            
...
def main():
    if len(sys.argv) < 4:
        print('Usage: python downloader.py {outdir} {threads:4} {url}')
        sys.exit(1)
    outdir, threads, url = sys.argv[1:4]
    obj = Downloader(url, threads, outdir)
    obj.start_download()
