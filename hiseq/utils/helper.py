...
def bam_to_bw(bam, outdir, scale=1.0, **args):
    ...
    genome_size = args.get('genome_size', 0)

    if genome in effsize:
        gsize = effsize[genome]
    elif genome_size > 0:
        gsize = genome_size
    else:
        gsize = 0
        #raise ValueError('genome=, genome_size=, not correct')
        log.error('genome=, genome_size=, not correct')

    ## command
    bamcoverage_exe = shutil.which('bamCoverage')
    if bamcoverage_exe is None:
        raise ValueError('{:10s} | program not found: bamCoverage'.format('failed'))
    ...
    log.info('create bigWig for: {}'.format(prefix))
    ...
    if args['filterRNAstrand'] is None:
        bw_file = os.path.join(outdir, prefix + '.bigWig')
        opt2 = ''
    else:
        if args['filterRNAstrand'] == 'forward':
            if args['library_type'] == 2:
                bw_file = os.path.join(outdir, prefix + '.rev.bigWig')
            else:
                bw_file = os.path.join(outdir, prefix + '.fwd.bigWig')
    ...
    ## opt
    opt3 = '--scaleFactor {} -b {} -o {} --binSize {} -p {}'.format(
        scale, bam, bw_file, args['binsize'], args['p'])
  
    ## opt
    if args['normalizeUsing']:
        opt3 += ' --normalizeUsing {}'.format(args['normalizeUsing'])
    ...
    if os.path.exists(bw_file) and not args['overwrite']:
        log.info('file exists, skipped: {}'.format(bam))
    else:
    ...
    if not os.path.exists(bw_file):
        raise Exception('bamCoverage failed, output not found: {}'.format(bw_file))
