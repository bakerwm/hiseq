...
from hiseq.chipseq.chipseq_rd import ChipseqRd
from hiseq.chipseq.chipseq_rx import ChipseqRx
...
class Chipseq(object):
    ...
    def init_args(self):
        args_local = ChipseqConfig(**self.__dict__)
        self = update_obj(self, args_local.__dict__, force=True)


    def run_rd(self):
        ChipseqRd(**self.__dict__).run()
    ...
    def run(self):
        if self.build_design:
            ...
            # ChipseqRx(**self.__dict__).run() # Rx->Rn->R1
...
class ChipseqConfig(object):
    ...
    def init_args(self):
        ...
        if not isinstance(self.design, str):
            log.error('--design, expect str, got {}'.format(
                type(self.design).__name__))


def get_args():
    """
    Parsing arguments for Chipseq
    """
    example = '\n'.join([
        'Examples:',
        '1. Generate design.toml, with --fq-dir',
        '$ python chipseq.py -b -d design.toml --fq-dir data/raw_data --ip K9 --input IgG',
        '2. Generate design.toml, with -1 and -2',
        '$ python chipseq_rd.py -b -d design.toml -1 *1.fq.gz -2 *2.fq.gz',
        '3. Run pipeline for design.toml',
        '$ python chipseq.py -d design.toml -g dm6 -o results',
        '4. Run pipeline, with different parameters',
        '$ python chipseq.py -d design.toml -g dm6 -o results --extra-index te',
    ])
    parser = argparse.ArgumentParser(
        prog='chipseq',
        description='chipseq: for multiple groups of PE reads',
        epilog=example,
