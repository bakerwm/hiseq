...
"""
Chipseq-seq pipeline: level-2 (generate report)
Generate report
"""
...
class ChipseqRp(object):
    ...
    >>> ChipseqRp(project_dir).run()
    >>> ChipseqRp(project_dir).report()

    $ python chipseq_rp.py -i prj_dir
    """
    ...
    def report(self):
        ...
        with open(cmd_txt, 'wt') as w:
            w.write(cmd + '\n')
        # report_html
        if file_exists(chipseq_report_html) and not self.overwrite:
...
def get_args():
    example = '\n'.join([
        'Examples:',
        '$ python chipseq_rp.py -i project_dir',
    ])
